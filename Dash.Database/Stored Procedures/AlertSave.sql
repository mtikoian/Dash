...
AS
	...
	IF ISNULL(@Id, 0) = 0
		BEGIN
			INSERT INTO Alert ([Name], [ReportId], [OwnerId], SendTo, [Subject], ResultCount, CronMinute, CronHour, CronDayOfMonth, CronMonth, CronDayOfWeek, LastRunDate, [Hash], IsActive, NotificationInterval, LastNotificationDate, UserCreated)
				VALUES (@Name, @ReportId, @OwnerId, @SendTo, @Subject, @ResultCount, @CronMinute, @CronHour, @CronDayOfMonth, @CronMonth, @CronDayOfWeek, @LastRunDate, @Hash, @IsActive, @NotificationInterval, @LastNotificationDate, @RequestUserId)
			SET @Id = SCOPE_IDENTITY()
		END
	ELSE
		BEGIN
			UPDATE Alert SET [Name] = @Name, [ReportId] = @ReportId, SendTo = @SendTo, [Subject] = @Subject, ResultCount = @ResultCount,  
